...
> 本文对旧实现错误的判断保持不变。
>
> 2026-07-17 补充：这里的“占位”只指源码中的 `0.90/0.85` 两个数值，
> 并不表示 15 张历史 PNG 没有运行过。图片、索引偏差、rFFT 丢虚部
> 复现和正确 DCT 五种子结果见
> [`freqkv_reassessment_20260717.md`](freqkv_reassessment_20260717.md)。
>
> 2026-07-17 再补充：FFT_Com 已独立完成 Qwen2.5-1.5B 真实 KV
> 频谱探针。去序列均值后，RoPE 后 K 的 DCT 低频 25% 能量为
> `65.51%`，V 为 `43.97%`。这使真实 K 方向值得继续，但不改变
> 对旧 FreqKV 代码、占位数字和部署结论的审计。
...
历史 `freqkv_final_results` 的 15 张 PNG 是实际执行产物，但图中使用的
是 rFFT-shaped 频率域。seq64 正确 DCT 五种子复核中，低频 25% 对 K/V
只保留 `0.25067/0.24762` 能量；历史 rFFT 丢虚部路径在请求保留 90%
时只报告 `0.45464`。此外频谱图把 `searchsorted` 的零基 index 直接
除以分量数，图例少计了一个实际所需分量。
...
判断：这是确实运行过、但结论错误且未完成的合成原型。可以保留设计
问题与重启要求，但不应继续传播其效果结论。

独立新探针已经补上“真实 KV 是否存在频谱结构”的第一层证据：
RoPE 后 K 的原始/去均值低频 25% 能量为 `85.49%/65.51%`，V 为
`52.69%/43.97%`。它尚未补上压缩注入、任务质量、compact payload、
内存和延迟，因此结论是“继续真实 K 验证”，不是恢复旧效果主张。
